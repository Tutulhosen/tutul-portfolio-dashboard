...
<main class="main-content p-3">
    ...
    <div class="card">
        <div class="card-body">
            <a href="{{route('achievement.create')}}" class="btn btn-primary">Create new ++</a>
            <h4 class="text-center">Achievement </h4>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($achievement->attached_file)
                                        <img style="height: 70px; width:70px; object-fit:cover" src="{{asset('storage/' . $achievement->attached_file)}}" alt=""></td>  
                                    @endif
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        ...
                                        <button class="btn btn-danger btn-sm delete-btn" data-achievement-id="{{ $achievement->id }}">
                                    ...
                                    <form id="delete-form-{{ $achievement->id }}" action="{{ route('achievement.destroy', $achievement->id) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
...
<script>
    ...
    $(document).on('click', '.status_btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `/achievement/status/${achievementId}`,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
    ...
    $(document).on('click', '.delete-btn', function () {
        let achievementId = $(this).data('achievement-id'); 
        let deleteForm = $('#delete-form-' + achievementId); 
